...
describe('Тестирование конструктора неавторизованным пользователем', () => {
  ...
  it('Кейс добавления булки в конструктор', () => {
    ...
    cy.get('[data-testid="bun-top"]').contains(firstBunFixture.name + ' (верх)');

    // Проверяем, что булка появилась в списке
    cy.get('[data-testid="bun-bottom"]').contains(firstBunFixture.name + ' (низ)');
  ...
  it('Кейс добавления ингредиента в конструктор', () => {
    ...
    cy.get(addedMain)
      .first()
      .should('contain.text', firstMainFixture.name);
...
describe('Оформления заказа авторизованным пользователем', () => {
  ...
  it('Кейс отправки заказа авторизованным пользователем', () => {
    ...
    cy.get('[data-testid="order-number"]').should('have.text', orderFixture.order.number);
